...
echo [3/6] Spring Boot 백엔드 시작 중... (포트 8080)
...
echo [4/6] 최근 문서 DB 서버 시작 중... (포트 8002)
...
REM OCR 서버 시작 (포트 8001)
echo [5/6] OCR 서버 시작 중... (포트 8001)
start "EasyDOC OCR Server" powershell -ExecutionPolicy Bypass -NoExit -Command "cd backend-OCR; .\venv\Scripts\python ocr_server.py"

REM 프론트엔드 시작 (포트 5173)
echo [6/6] 프론트엔드 시작 중... (포트 5173)
